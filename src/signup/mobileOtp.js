...
/**
 * Signup screen to input otp to verify the received mobile number
 */
function MobileOtp({ setValidator }) {
    const { formData, updatePageState } = useContext(SignupContext);
    const [valid, setValid] = useState(false);
    const [callBack, setCallBack] = useState(() => { return false });

    useEffect(() => {
        getOtp({ "mobile": formData.mobile });
